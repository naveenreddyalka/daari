    ...
    def router(self) -> APIRouter:
        ...
        async def chat_completions(
            ...
            if group_denied is not None:
                return group_denied
            model_denied = reject_model_max_budget(request, body.model, ctx.settings)
        ...
        async def rerank(body: dict[str, Any], request: Request) -> Any:
            """Cohere/LiteLLM-shaped rerank via configured L6 (#1051)."""
            from daari.gateway.rerank import RerankRequest, handle_rerank
        ...
        async def daari_report(request: Request, days: int = 7) -> dict[str, Any]:
            ...
            payload["team_model_spend"] = team_model_spend_report_rows(
                store,
                ledger,
                soft_ratio=float(ctx.settings.frontier.soft_budget_ratio or 0.0),
                pricing=getattr(ctx.settings, "pricing", None),
