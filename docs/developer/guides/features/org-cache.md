...
| `daari cache invalidate --token <bearer>` | When `enterprise.sso` is enabled and `daari serve` is up, send `Authorization: Bearer` (SSO token or master key). If omitted, the CLI uses `server.api_key` when configured. |
