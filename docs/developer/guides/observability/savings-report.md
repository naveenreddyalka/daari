...
daari report --format json          # same payload as GET /v1/daari/report
daari usage --by-user
...
markdown `daari report` print that table; `daari report --format json` dumps
the full HTTP JSON (including `totals`, `frontier`, and `request_quotas`) for
