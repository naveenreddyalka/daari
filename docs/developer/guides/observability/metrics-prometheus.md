...
| `daari_cancelled_requests_total{phase}` | Client hung up (`phase` is `chat`, `anthropic`, `responses`, or `stream`) and the upstream call was cancelled. Normal completions do not increment. |
