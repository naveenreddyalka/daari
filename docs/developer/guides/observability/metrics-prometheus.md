...
| `daari_cancelled_requests_total{phase}` | Client hung up (`phase` is `chat`, `anthropic`, `responses`, `stream`, `embed`, `asr`, or `translation`) and the upstream call was cancelled. Normal completions do not increment. |
