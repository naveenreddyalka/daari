...
Helm fleets can set `asr.baseUrl`, which mounts `DAARI_ASR__BASE_URL` (see [Capacity and Helm](../operations/capacity-helm.md)). `asr.frontierFallback` defaults off and omits env; set it to true to mount `DAARI_ASR__FRONTIER_FALLBACK` so an empty base URL may forward one transcription to frontier.
