...
**v1.3.0 released (2026-08-17) — Gateway Completeness, Hardening & Relicense.** Full agent gateway surface (complete Responses API, MCP server, embeddings, vision, native Anthropic egress), distributed RPM/TPM rate limits, health-checked local backend pools, real token accounting, and a relicense to PolyForm Noncommercial 1.0.0 (later returned to Apache 2.0 on `main` — [ADR-0016](docs/adr/0016-apache-2-relicense.md)). See [docs/RELEASE-v1.3.0.md](docs/RELEASE-v1.3.0.md). Roadmap v1 Phases A–E are all shipped at least to tracer depth: full tier chain (L0/L1/CCS/L2/Lt/L3–L6), prompt intelligence + traces + savings ledger, trust trains (false-hit measurement, prompt-cache passthrough, latency-aware + learned routing, budgets/PII scrub), Phase D learning (feedback → tuner → MLX fine-tune → deploy, opt-in D3 stats export), one-click clients (Cursor BYOK, Claude Code with full tool passthrough, JetBrains via Ollama facade, VS Code), per-project `.daari.yaml` profiles, gateway API-key auth, MLX backend, org shared cache/learning (E2/E3 tracer).
...
Apache 2.0 **local cost optimizer** — routes work through cache → tools → local AI before frontier.
