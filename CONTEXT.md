...
**v1.4.0 prepared (2026-09-03) — Apache 2.0, Supply Chain & Enterprise Gateway.** Version bump and [RELEASE-v1.4.0.md](docs/RELEASE-v1.4.0.md) ready for human `git tag` / `gh release create` (AGENTS.md hard limit). Tree is Apache 2.0 ([ADR-0016](docs/adr/0016-apache-2-relicense.md)); ghcr images gain cosign + SBOM + provenance. Prior tagged release: **v1.3.0** (2026-08-17, PolyForm NC as tagged) — [docs/RELEASE-v1.3.0.md](docs/RELEASE-v1.3.0.md). Roadmap v1 Phases A–E are all shipped at least to tracer depth: full tier chain (L0/L1/CCS/L2/Lt/L3–L6), prompt intelligence + traces + savings ledger, trust trains (false-hit measurement, prompt-cache passthrough, latency-aware + learned routing, budgets/PII scrub), Phase D learning (feedback → tuner → MLX fine-tune → deploy, opt-in D3 stats export), one-click clients (Cursor BYOK, Claude Code with full tool passthrough, JetBrains via Ollama facade, VS Code), per-project `.daari.yaml` profiles, gateway API-key auth, MLX backend, org shared cache/learning (E2/E3 tracer).
